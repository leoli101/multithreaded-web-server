...
HWSize_t WriteIndex(MemIndex mi, DocTable dt, const char *filename) {
  ...
  HWSize_t hdres = WriteHeader(f, dtres, mires);
  // check if the header is exactly 16-byte.
  if (hdres != 16) {
    fclose(f);
    unlink(filename);
    return 0;
...
static HWSize_t WriteDocPositionListFn(FILE *f,
  ...
  if (fseek(f, offset, SEEK_SET) != 0)
    return 0;
  res = fwrite(reinterpret_cast<char *>(&header), sizeof(header), 1, f);
  if (res != 1)
    return 0;
  ...
  for (i = 0; i < num_pos_ho; i++) {
    ...
    // DocPositionOffset_t position32_t = position.position;
    // position32_t = htonl(position32_t);
    position.toDiskFormat();
    fwrite(reinterpret_cast<char *>(&position), sizeof(position), 1, f);
    if (res != 1)
      return 0;
...
static HWSize_t WriteHeader(FILE *f,
  ...
  // from fileindexutil.h to do the CRC checksum calculation,
  // feeding it characters that you read from the index file using fread().
  ...
  HWSize_t i;
  for (i = 0; i < cslen; i++) {
    res = fread(reinterpret_cast<void *>(&cur_byte), sizeof(cur_byte), 1, f);
    Verify333(res == 1);
    crcobj.FoldByteIntoCRC(cur_byte);
  }
  uint32_t crc_res = crcobj.GetFinalCRC();
  header.checksum = crc_res;
...
static HWSize_t WriteBucketRecord(FILE *f,
  ...
  res = fwrite(reinterpret_cast<char *>(&br), sizeof(br), 1, f);
  if (res != 1)
    return 0;
...
static HWSize_t WriteBucket(FILE *f,
  ...
  if (chainlen_ho > 0) {
    ...
    for (j = 0; j < chainlen_ho; j++) {
      ...
      // STEP 17.
      // See comment above "if (chainlen_ho > 0)" for instructions.
...
static HWSize_t WriteHashTable(FILE *f,
  ...
  for (i = 0; i < ht->num_buckets; i++) {
    // STEP 18.
    HWSize_t ll_len = NumElementsInLinkedList(ht->buckets[i]);
    res = WriteBucketRecord(f, ht->buckets[i],
