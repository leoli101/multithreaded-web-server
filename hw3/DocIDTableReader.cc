...
bool DocIDTableReader::LookupDocID(const DocID_t &docid,
  ...
  for (auto it = elements.begin(); it != elements.end(); it++) {
    ...
    size_t res;
    res = fread(reinterpret_cast<void *>(&header),
    ...
    if (header.docid == docid) {
      ...
      docid_element_position next_pos;
      for (i = 0; i < header.num_positions; i++) {
        res = fread(reinterpret_cast<void *>(&next_pos),
                   sizeof(docid_element_position),
                   1,
                   file_);
        Verify333(res == 1);
        next_pos.toHostFormat();
        list.push_back(next_pos.position);
      }
...
list<docid_element_header> DocIDTableReader::GetDocIDList() {
  ...
  for (HWSize_t i = 0; i < header_.num_buckets; i++) {
    ...
    bucketrec_offset = offset_ + sizeof(header_) + i * sizeof(bucket_rec);
    ...
    bucket_rec b_rec;
    size_t res;
    res = fread(reinterpret_cast<void *>(&b_rec),
               sizeof(bucket_rec),
               1,
               file_);
    b_rec.toHostFormat();
    ...
    for (HWSize_t j = 0; j < b_rec.chain_len; j++) {
      ...
      res = fread(reinterpret_cast<void *>(&ep),
      ...
      res = fread(reinterpret_cast<void *>(&doc_el),
                 sizeof(docid_element_header),
                 1,
                 file_);
      doc_el.toHostFormat();
