  ...
  while (1) {
    ...
    if (res == 0) {
      ...
      if (S_ISREG(nextstat.st_mode)) {
        ...
        // Open the directory and check error.
        DIR *new_dir = opendir(newfile);
        // If the directory can not be open
        if (new_dir == NULL) {
          free(newfile);
          fprintf(stderr, "Failed to open directory: %s\n", newfile);
          return;
        }
        HandleDir(newfile, new_dir, doctable, index);
        res = closedir(new_dir);
        Verify333(res == 0);
      }
    }
...
static void HandleFile(char *fpath, DocTable *doctable, MemIndex *index) {
  ...
  while (NumElementsInHashTable(tab) > 0) {
    ...
    int res;
    res = MIAddPostingList(*index, (char*)wp->word, docID, wp->positions);
    Verify333(res == 1);
